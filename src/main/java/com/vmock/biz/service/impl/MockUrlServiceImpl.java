...
public class MockUrlServiceImpl extends ServiceImpl<MockUrlMapper, MockUrl> implements IMockUrlService {
	...
	public List<MockUrl> selectMockUrlList(MockUrl mockUrl) {
		return this.list(Wrappers.<MockUrl>lambdaQuery()
				.like(StrUtil.isNotBlank(mockUrl.getUrl()), MockUrl::getUrl, mockUrl.getUrl())
	...
	public MockUrl getUrlByRegexp(String requestUrlLogic) {
		if (StrUtil.isNotBlank(requestUrlLogic)) {
			...
			regexpJoiner.add(CommonConst.PATH_PLACEHOLDER.equals(item) ? SQL_REGEXP_PATH
					: StrUtil.format(SQL_REGEXP_FORMAT, item)));
	...
	@Override
	public List<MockUrl> selectMockUrlList(String url) {
		return this.list(Wrappers.<MockUrl>lambdaQuery().like(StrUtil.isNotBlank(url), MockUrl::getUrl, url)
				.orderByAsc(MockUrl::getCreateTime));
	}
}
