...
public class MockFilterHandler {
	...
	public boolean execute(HttpServletRequest request, HttpServletResponse response) {
		...
		// 4. Additional path retrive
		mockUrl = matchUrlbyRegexp(mockUrlService.selectMockUrlList("/"), requestUrl);
		// 如果还是没有url, 那是真没有了
		if (mockUrl == null) {
		...
		// 方法校验 ----
		if (!request.getMethod().equalsIgnoreCase(mockUrl.getMethod())) {
	...
	private MockUrl matchUrlbyRegexp(List<MockUrl> selectMockUrlList, String requestUrl) {
		MockUrl mockUrl = null;
		if (selectMockUrlList != null && selectMockUrlList.size() > 0) {
			String[] paths = requestUrl.split("/");
			boolean flag = true;
			for (MockUrl url : selectMockUrlList) {
				String[] tempPaths = url.getUrl().split("/");
				if (paths.length != tempPaths.length) {
					flag = false;
					continue;
				}
				for (int i = 0; i < paths.length; i++) {
					if (!paths[i].contains("{") && !paths[i].contentEquals(tempPaths[i])) {
						flag = false;
						break;
					}
				}
	 ...
	 * @param response
	 */
	private void successAndLog(MockUrl mockUrl, MockResponse mockResponse, HttpServletRequest request,
